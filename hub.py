...
:root { --bg-dark: #0a0a0a; --bg-gradient: radial-gradient(circle at 50% 0%, #1f1f1f 0%, #0a0a0a 70%); --glass-bg: rgba(255, 255, 255, 0.03); --glass-border: rgba(255, 255, 255, 0.1); --glass-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.5); --accent: #ffffff; --accent-glow: rgba(255, 255, 255, 0.3); --accent-text: #000000; --neon-red: #ef4444; --neon-red-glow: rgba(239, 68, 68, 0.4); --neon-orange: #f97316; --neon-orange-glow: rgba(249, 115, 22, 0.4); --text-main: #f8fafc; --text-muted: #94a3b8; --input-bg: rgba(0,0,0,0.4); }
[data-theme='black-blue'] { --bg-dark: #0a0a0f; --bg-gradient: radial-gradient(circle at 50% 0%, #1a1a2e 0%, #0a0a0f 70%); --accent: #3b82f6; --accent-glow: rgba(59, 130, 246, 0.4); --accent-text: #ffffff; }
[data-theme='black-red'] { --bg-dark: #0f0000; --bg-gradient: radial-gradient(circle at 50% 0%, #2a0808 0%, #0f0000 70%); --accent: #ef4444; --accent-glow: rgba(239, 68, 68, 0.4); --accent-text: #ffffff; }
[data-theme='pure-black'] { --bg-dark: #000000; --bg-gradient: none; --glass-bg: #000000; --glass-border: #333333; --glass-shadow: none; --accent: #ffffff; --accent-glow: rgba(255, 255, 255, 0.1); --accent-text: #000000; --text-main: #e5e5e5; --text-muted: #737373; --input-bg: #000000; }
[data-theme='light'] { --bg-dark: #f8fafc; --bg-gradient: radial-gradient(circle at 50% 0%, #ffffff 0%, #e2e8f0 100%); --glass-bg: rgba(255, 255, 255, 0.6); --glass-border: rgba(0, 0, 0, 0.1); --glass-shadow: 0 8px 32px 0 rgba(0, 0, 0, 0.05); --accent: #0f172a; --accent-glow: rgba(15, 23, 42, 0.2); --accent-text: #ffffff; --text-main: #0f172a; --text-muted: #475569; --input-bg: rgba(255,255,255,0.8); }
body { font-family: 'Inter', system-ui, sans-serif; background: var(--bg-dark); background-image: var(--bg-gradient); color: var(--text-main); margin: 0; min-height: 100vh; transition: background 0.3s ease, color 0.3s ease; }
.glass-box { background: var(--glass-bg); backdrop-filter: blur(16px); -webkit-backdrop-filter: blur(16px); border: 1px solid var(--glass-border); border-radius: 16px; box-shadow: var(--glass-shadow); transition: 0.3s; }
...
.badge { border: 1px solid var(--accent); padding: 4px 14px; border-radius: 50px; font-size: 11px; font-weight: 600; color: var(--accent); background: rgba(128,128,128,0.1); text-transform: uppercase; }
...
.btn { padding: 8px 16px; border-radius: 10px; font-size: 13px; font-weight: 600; cursor: pointer; border: 1px solid var(--glass-border); background: var(--glass-bg); color: var(--text-main); transition: 0.3s; }
...
.tree-item { padding: 12px 15px; cursor: pointer; border-radius: 8px; color: var(--text-muted); font-size: 14px; }
...
<body>
    ...
    <div class="container">
        ...
        <div style="display:flex;justify-content:space-between;margin-bottom:20px">
            <div style="font-size:14px;color:var(--text-muted)">{breadcrumbs}</div>
            <div style="display:flex;gap:12px">{admin_top_btn}{admin_log_btn}</div>
        </div>
        {disk_dashboard}
        {admin_upload_area}
    ...
    <div id="batch-bar" class="glass-box" style="display:none;position:fixed;bottom:25px;left:50%;transform:translateX(-50%);z-index:1001;padding:15px 25px;align-items:center;gap:15px;border-color:var(--accent)">
        <span id="batch-count" style="font-weight:900;color:var(--accent)">0 selected</span>
    ...
    <div id="previewModal" class="modal"><div class="modal-content"><span style="position:absolute;top:-40px;right:0;color:#fff;font-size:35px;cursor:pointer" onclick="closePreview()">&times;</span><div id="previewBody" style="width:100%;height:100%;display:flex;justify-content:center;align-items:center"></div></div></div>
    <div id="treeModal" class="modal"><div class="modal-content glass-box" style="flex-direction:column;padding:25px;max-width:500px"><h3 id="tree-title" style="margin:0 0 20px 0;color:var(--text-main)">Select Destination</h3><div id="tree-list" style="flex:1;width:100%;overflow-y:auto;background:var(--input-bg);border:1px solid var(--glass-border);border-radius:12px;padding:15px"></div><div style="margin-top:20px;display:flex;gap:12px;width:100%;justify-content:flex-end"><button class="btn" onclick="document.getElementById('treeModal').style.display='none'">Cancel</button><button class="btn btn-action" onclick="confirmTreeAction()">Confirm</button></div></div></div>
    <div id="logModal" class="modal"><div class="modal-content glass-box" style="flex-direction:column;padding:25px"><div style="display:flex;justify-content:space-between;width:100%;margin-bottom:20px"><h3 style="margin:0;color:var(--text-main)">System Logs</h3><div style="display:flex;gap:12px"><a href="/download_logs" class="btn">📥 Download</a><button class="btn" style="color:var(--neon-red)" onclick="clearLogs()">🗑️ Clear</button></div></div><textarea readonly id="log-viewer" style="width:100%;height:100%;background:rgba(0,0,0,0.8);color:#10b981;border-radius:12px;padding:20px"></textarea><button class="btn" style="margin-top:20px" onclick="document.getElementById('logModal').style.display='none'">Close</button></div></div>
    <div id="editModal" class="modal"><div class="modal-content glass-box" style="flex-direction:column;padding:25px"><h3 id="edit-name" style="color:var(--text-main)"></h3><textarea id="edit-box" style="width:100%;height:100%;background:rgba(0,0,0,0.8);color:#f8fafc;padding:20px"></textarea><div style="margin-top:20px;display:flex;gap:12px;justify-content:flex-end"><button class="btn" onclick="document.getElementById('editModal').style.display='none'">Cancel</button><button class="btn action-orange" onclick="saveEdit()">💾 Save</button></div></div></div>
    <script>
        const currentDir="{current_dir}";
        document.getElementById('themeSelector').value=localStorage.getItem('hub_theme')||'black-white';
        function changeTheme(t){{ document.documentElement.setAttribute('data-theme',t); localStorage.setItem('hub_theme',t); }}
        
        let selectedFiles=[];
        function toggleSelection(e){{ e.stopPropagation(); updateBatchBar(); }}
        function toggleAll(e){{ document.querySelectorAll('.file-cb').forEach(cb=>cb.checked=e.target.checked); updateBatchBar(); }}
        function updateBatchBar(){{
            selectedFiles=Array.from(document.querySelectorAll('.file-cb:checked')).map(cb=>cb.value);
            let b=document.getElementById('batch-bar');
            if(selectedFiles.length>0){{ b.style.display='flex'; document.getElementById('batch-count').innerText=selectedFiles.length+" selected"; }}
            else{{ b.style.display='none'; }}
        }}
        function batchDelete(){{ if(confirm('Delete '+selectedFiles.length+' items?')) fetch('/action',{{method:'POST',body:new URLSearchParams({{action:'batch_delete',targets:selectedFiles.join('|'),dir:currentDir}})}}).then(()=>location.reload()); }}
        ...
        function handleItemClick(u,t,lId){{
            if(lId){{
                ...
                let p=prompt("🔒 Locked. Password:");
                if(p) document.cookie="lock_"+lId+"="+p+";path=/"; else return;
            }}
            if(t==='download') window.location.href=u; else openPreview(u,t);
        }}
        ...
        window.onclick=(e)=>{{ if(!e.target.closest('.dropdown-content')&&!e.target.matches('.kebab-btn')) document.querySelectorAll('.dropdown-content').forEach(d=>d.classList.remove('show')); }};
        
        function openPreview(u,t){{
            let b=document.getElementById('previewBody'); b.innerHTML='';
        ...
        function openTreeModal(a,t){{
            treeAct=a; treeTgt=t; treeSel=null;
            document.getElementById('tree-title').innerText=(a.includes('move')?'✂️ Move ':'📄 Copy ')+(a.includes('batch_')?selectedFiles.length+' items':t);
            document.getElementById('treeModal').style.display='flex';
            fetch('/action',{{method:'POST',body:new URLSearchParams({{action:'get_tree'}})}}).then(r=>r.json()).then(ds=>{{
                let h='';
                ds.forEach(d=>{{
                    let pad=d==='/'?0:(d.split('/').length-1)*20;
                    let n=d==='/'?'Root ( / )':d.split('/').pop();
        ...
        function selectTreeItem(el,p){{ document.querySelectorAll('.tree-item').forEach(i=>i.classList.remove('selected')); el.classList.add('selected'); treeSel=p==='/'?'':p.substring(1); }}
        function confirmTreeAction(){{
            if(treeSel===null) return alert('Select destination!');
            fetch('/action',{{method:'POST',body:new URLSearchParams({{action:treeAct,[treeAct.startsWith('batch_')?'targets':'target']:treeTgt,dir:currentDir,dest:treeSel}})}}).then(()=>location.reload());
        }}
        ...
        function clearLogs(){{ if(confirm('Clear logs?')) callAct('clear_logs',''); }}
        function createFolder(){{ let n=prompt("New Folder:"); if(n) callAct('mkdir',n); }}
        function createFile(){{ let n=prompt("New File:"); if(n) callAct('mkfile',n); }}
        function deleteItem(n){{ if(confirm('Delete '+n+'?')) callAct('delete',n); }}
        function renameItem(n){{ let nn=prompt("Rename:",n); if(nn&&nn!==n) callAct('rename',n,{{new_name:nn}}); }}
        function lockItem(n){{ let p=prompt("Password (empty to unlock):"); if(p!==null) callAct('lock_item',n,{{pwd:p}}); }}
        function toggleDl(n){{ callAct('toggle_dl',n); }}
        
        function askPath(a,t,e={{}}){{
            let cp=prompt("Custom link path (empty for random):");
            if(cp===null) return;
            let p={{action:a,target:t,dir:currentDir,custom_path:cp}};
            Object.assign(p,e);
            fetch('/action',{{method:'POST',body:new URLSearchParams(p)}}).then(r=>r.text()).then(l=>{{
                if(l==="EXISTS") alert("⚠️ Path exists!");
                else {{ prompt("Link:",window.location.origin+l); location.reload(); }}
            }});
        }}
        ...
        function pwdShareItem(n){{ let p=prompt("Password:"); if(p) askPath('share_pwd',n,{{pwd:p}}); }}
        function renewItem(n){{ if(confirm('Renew?')) askPath('renew',n); }}
        function unshareItem(n){{ callAct('unshare',n); }}
        function viewLink(tk){{ prompt("Link:",window.location.origin+"/p/"+tk); }}
        ...
        function editItem(n,lId){{
            ...
            fetch('/download/'+currentDir+'/'+n).then(r=>r.text()).then(t=>{{
                document.getElementById('edit-name').innerText="📝 "+n;
                document.getElementById('edit-box').value=t;
                document.getElementById('edit-box').setAttribute('data-target',n);
                document.getElementById('editModal').style.display='flex';
            }});
        }}
        function saveEdit(){{ callAct('save_text',document.getElementById('edit-box').getAttribute('data-target'),{{content:document.getElementById('edit-box').value}}); }}
        
        const dz=document.getElementById('drop-zone');
        if(dz){{
            let i=document.getElementById('file-input'), dt=document.getElementById('drop-text'), sf=document.getElementById('selected-files');
            let btnStart=document.getElementById('btn-start-upload'), btnPublish=document.getElementById('btn-confirm-publish'), pf=[];
            
            dz.onclick=(e)=>{{ if(e.target!==btnStart && e.target!==btnPublish && !sf.contains(e.target)) i.click(); }};
            ...
            dz.ondrop=(e)=>{{ e.preventDefault(); dz.style.borderColor="var(--glass-border)"; if(e.dataTransfer.files.length>0){{ pf=Array.from(e.dataTransfer.files); showP(); }} }};
            i.onchange=(e)=>{{ if(e.target.files.length>0){{ pf=Array.from(e.target.files); showP(); }} }};
            
            function showP(){{
                dt.style.display='none'; sf.style.display='block'; 
                btnStart.style.display='block'; btnPublish.style.display='none';
                document.getElementById('progress-wrapper').style.display='none';
                document.getElementById('progress-bar').style.width='0%';
                sf.innerHTML=pf.map(f=>`📄 ${{f.name}} <span style="color:var(--text-muted)">(${{(f.size/1048576).toFixed(2)}} MB)</span>`).join('<br>');
            }}
            ...
            btnStart.onclick=(e)=>{{
                ...
                btnStart.style.pointerEvents='none'; btnStart.innerText='⏳ Uploading... Please wait'; sf.style.opacity='0.5';
                const fd=new FormData(); for(let f of pf) fd.append('file',f);
                
                document.getElementById('progress-wrapper').style.display='block';
                const x=new XMLHttpRequest(); x.open('POST','/upload?dir='+encodeURIComponent(currentDir),true);
                x.upload.onprogress=(ev)=>{{ document.getElementById('progress-bar').style.width=Math.round((ev.loaded/ev.total)*100)+'%' }};
                x.onload=()=>{{ 
...
def get_icon(f, is_d):
    ...
    if ext in ['zip', 'rar', '7z', 'tar']: return "📦"
    if ext in ['py', 'js', 'html', 'css', 'json', 'sh']: return "💻"
    return "📄"

def get_preview_type(f):
    ext = f.split('.')[-1].lower()
    if ext in ['jpg', 'png', 'gif', 'webp']: return 'image'
...
class FileHubHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if check_ip(self.client_address[0]): return self._send_resp('<h1>🚫 BLOCKED</h1>')
        ...
        if parsed.path.startswith("/p/"):
            ...
            if tk in lns:
                ld = lns[tk]; t_rel = ld.get('target') if isinstance(ld, dict) else ld
        ...
        role = self.get_role()
        if not role: return self._send_resp(LOGIN_HTML.replace('{site_name}', self.CONFIG.get('SITE_NAME', 'HUB')))
        ...
        if parsed.path.startswith("/zip/"):
            ...
            if os.path.isdir(tgt):
                tmp = tempfile.mktemp(); shutil.make_archive(tmp, 'zip', tgt)
                self._send_file(tmp+'.zip', dl=True, name=os.path.basename(tgt)+".zip"); os.remove(tmp+'.zip')
            return
        ...
        if parsed.path == "/": 
            ...
            self.send_response(302); self.send_header("Set-Cookie", "auth=; Max-Age=0; Path=/;"); self.send_header("Location", "/"); self.end_headers()
    ...
    def do_POST(self):
        if check_ip(self.client_address[0]): return self.send_error(403)
        parsed = urllib.parse.urlparse(self.path)
        ...
        if parsed.path == "/login":
            ...
            if pwd in [self.CONFIG['ADMIN_PWD'], self.CONFIG['GUEST_PWD']]:
                ...
                self.send_response(302); self.send_header("Set-Cookie", f"auth={tk}; Path=/; HttpOnly"); self.send_header("Location", "/"); self.end_headers()
            else: 
        ...
        if parsed.path == "/upload": return self._handle_upload(curr)
            ...
            if act == 'get_logs':
                ...
                self.send_response(200); self.end_headers(); self.wfile.write(c.encode()); return
                
            if act == 'get_tree':
                b = os.path.abspath(self.CONFIG['UPLOAD_DIR']); dirs = ["/"]
                for r, dn, _ in os.walk(b):
                    for dp in dn: dirs.append("/" + os.path.relpath(os.path.join(r, dp), b).replace('\\', '/'))
                self.send_response(200); self.end_headers(); self.wfile.write(json.dumps(sorted(dirs)).encode()); return

            if act == 'clear_logs':
                open(LOG_FILE, 'w').close(); self.send_response(200); self.end_headers(); return
            
            tp = os.path.join(curr, tgt); rel = self.get_rel(tp)
            ...
            if act in ['batch_delete', 'batch_move', 'batch_copy']:
                ...
                for t in d.get('targets', [''])[0].split('|'):
                    ...
                    if act == 'batch_delete' and os.path.exists(tm):
                        ...
                        shutil.move(tm, f) if act == 'batch_move' else (shutil.copytree if os.path.isdir(tm) else shutil.copy2)(tm, f)
                self.send_response(200); self.end_headers(); return
            
            if act == 'mkdir': os.makedirs(tp, exist_ok=True)
            elif act == 'mkfile' and not os.path.exists(tp): open(tp, 'w').close()
                ...
                nn = d.get('new_name', [''])[0]; nt = os.path.join(curr, nn)
                if nn and not os.path.exists(nt): os.rename(tp, nt)
            elif act in ['move', 'copy'] and os.path.exists(tp):
                dest = os.path.join(self.get_safe_path(d.get('dest', [''])[0]), tgt)
                shutil.move(tp, dest) if act == 'move' else (shutil.copytree if os.path.isdir(tp) else shutil.copy2)(tp, dest)
                ...
                locks = load_json(LOCKS_FILE); pwd = d.get('pwd', [''])[0]
                ...
                if cp:
                    if cp in lns and act != 'renew': self.send_response(200); self.end_headers(); self.wfile.write(b"EXISTS"); return
                    tk = cp
                else:
                    tk = str(uuid.uuid4())[:8]; 
                    while tk in lns: tk = str(uuid.uuid4())[:8]
                lns[tk] = {'target': rel, 'limit': int(d.get('limit', ['-1'])[0]) if act == 'share_limit' else -1, 'pwd': d.get('pwd', [''])[0] if act == 'share_pwd' else ""}
    ...
    def _serve_ui(self, role, curr, req_dir):
        pts = [p for p in req_dir.split('/') if p]; bc = '<a href="/">Root</a>'; acc = ""
        for p in pts: acc += f"/{p}"; bc += f' / <a href="/?dir={urllib.parse.quote(acc)}">{p}</a>'
        
        sa_btn = '<label style="color:var(--text-main);font-size:12px;background:var(--glass-bg);padding:6px;border-radius:8px;"><input type="checkbox" onchange="toggleAll(event)"> Select All</label>' if role == 'admin' else ''
        ...
        up_a = '<div class="glass-box" id="drop-zone" style="padding:25px;text-align:center;margin-bottom:25px;cursor:pointer;border:2px dashed var(--glass-border);"><p id="drop-text">☁️ Drag & Drop or click to select files</p><input type="file" id="file-input" hidden multiple><div id="selected-files" style="display:none;margin-top:15px;font-size:13px;max-height:100px;overflow-y:auto;text-align:left;padding:10px;background:rgba(0,0,0,0.3)"></div><button id="btn-start-upload" class="btn btn-action" style="display:none;margin-top:15px;width:100%">🚀 Start Upload</button><button id="btn-confirm-publish" class="btn" style="display:none;margin-top:15px;width:100%;background:#10b981;color:white;border-color:#10b981;box-shadow:0 0 20px rgba(16, 185, 129, 0.4);">✅ Confirm & Publish</button><div id="progress-wrapper" style="display:none;height:4px;background:rgba(0,0,0,0.5);margin-top:15px"><div id="progress-bar" style="width:0;height:100%;background:var(--accent)"></div></div></div>' if role == 'admin' else ''
        ...
        if role == 'admin':
            try:
                ...
                d_html = f'<div class="glass-box" style="display:flex;justify-content:space-around;padding:15px;margin-bottom:20px;text-align:center"><div>Space: <b>{format_size(tot)}</b></div><div style="color:#10b981">Free: <b>{format_size(fre)}</b></div><div style="color:var(--neon-orange)">Hub Size: <b>{format_size(dir_size)}</b></div></div>'
            except: pass

        lns, lks, ndl = load_json(LINKS_FILE), load_json(LOCKS_FILE), load_json(NODL_FILE)
        lmap = {}
        ...
        rows = f'<div class="file-item" data-name=".."><div class="file-info"><a href="/?dir={urllib.parse.quote("/".join(pts[:-1]))}" class="file-name">🔙 Return</a></div></div>' if pts else ""
        
        try: ents = sorted(list(os.scandir(curr)), key=lambda e: (not e.is_dir(), e.name.lower()))
        except: ents = []
        
        for e in ents:
            f = e.name
            if f in [CONFIG_FILE, LINKS_FILE, LOCKS_FILE, LOG_FILE, BLOCK_FILE, NODL_FILE, sys.argv[0].split('/')[-1]]: continue
            is_d = e.is_dir(); sz = format_size(e.stat().st_size) if not is_d else "--"
            dt = datetime.datetime.fromtimestamp(e.stat().st_mtime).strftime('%Y-%m-%d %H:%M'); rel = self.get_rel(e.path)
            ...
            s_bdg = f'<span style="color:#3b82f6;font-size:10px">👀 Stream Only</span>' if is_ndl else ""
            cb = f'<input type="checkbox" class="file-cb" value="{html.escape(f)}" onclick="toggleSelection(event)" style="margin-right:10px">' if role == 'admin' else ''
            
            if is_d:
                nx = f"{req_dir}/{f}".strip('/')
                ah = f'<button onclick="handleItemClick(\'/zip/{nx}\',\'download\',\'{l_id}\')">📦 ZIP</button><button onclick="lockItem(\'{f}\')">🔒 Lock</button><button onclick="renameItem(\'{f}\')">✏️ Rename</button><button onclick="copyItem(\'{f}\')">📄 Copy</button><button onclick="moveItem(\'{f}\')">✂️ Move</button><button style="color:red" onclick="deleteItem(\'{f}\')">🗑️ Delete</button>' if role == 'admin' else f'<button onclick="handleItemClick(\'/zip/{nx}\',\'download\',\'{l_id}\')">📦 ZIP</button>'
                rows += f'<div class="file-item" data-name="{f}"><div class="file-info">{cb}📁 <a href="/?dir={urllib.parse.quote(nx)}" class="file-name">{f}{l_inf}</a></div><div class="file-meta"><span>{dt}</span><span>{sz}</span></div><div class="actions"><button class="kebab-btn" onclick="toggleMenu(event, \'m-{f}\')">⋮</button><div class="dropdown-content" id="m-{f}">{ah}</div></div></div>'
            else:
                ptype = get_preview_type(f); dl = urllib.parse.quote(f"/{req_dir}/{f}".replace('//', '/'))
                ...
                if rel in lmap:
                    ...
                    sh_bdg = f'<span style="color:red;font-size:10px">● Shared{ph if role=="admin" else ""}</span>'
                    sh_btn = f'<button onclick="viewLink(\'{tk}\')">👁️ View Link</button><button onclick="renewItem(\'{f}\')">🔄 Renew</button><button style="color:red" onclick="unshareItem(\'{f}\')">🚫 Unshare</button>'
                else:
                    sh_btn = f'<button onclick="shareItem(\'{f}\')">🔗 Public Link</button><button onclick="limitedShareItem(\'{f}\')">⏳ Limited Link</button><button onclick="pwdShareItem(\'{f}\')">🔑 Secure Link</button>'
                
                if role == 'admin':
                    ebtn = f'<button onclick="editItem(\'{f}\',\'{l_id}\')">📝 Edit</button>' if f.split('.')[-1].lower() in ['txt','py','json','html','css','sh'] else ""
                    ah = f'<button onclick="toggleDl(\'{f}\')">{"✅ Enable DL" if is_ndl else "🚫 Disable DL"}</button>{sh_btn}{ebtn}<button onclick="lockItem(\'{f}\')">🔒 Lock</button><button onclick="renameItem(\'{f}\')">✏️ Rename</button><button onclick="copyItem(\'{f}\')">📄 Copy</button><button onclick="moveItem(\'{f}\')">✂️ Move</button><button style="color:red" onclick="deleteItem(\'{f}\')">🗑️ Delete</button>'
                else: ah = ''
                
                dlb = f'<button onclick="handleItemClick(\'/download{dl}&dl=1\',\'download\',\'{l_id}\')" class="btn">DL</button>' if role == 'admin' or not is_ndl else ''
                rows += f'<div class="file-item" data-name="{f}"><div class="file-info">{cb}{get_icon(f, False)} <span onclick="{pcl}" class="file-name">{f}{l_inf}{sh_bdg}{s_bdg}</span></div><div class="file-meta"><span>{dt}</span><span>{sz}</span></div><div class="actions">{dlb}<button class="kebab-btn" onclick="toggleMenu(event, \'m-{f}\')">⋮</button><div class="dropdown-content" id="m-{f}">{ah}</div></div></div>'
        
        hout = UI_HTML.replace('{site_name}', self.CONFIG.get('SITE_NAME', 'HUB')) \
                      .replace('{role}', role.capitalize()).replace('{breadcrumbs}', bc) \
                      .replace('{admin_top_btn}', sa_btn+a_btn).replace('{admin_log_btn}', l_btn) \
                      .replace('{disk_dashboard}', d_html).replace('{admin_upload_area}', up_a) \
                      .replace('{file_rows}', rows).replace('{current_dir}', req_dir)
        self._send_resp(hout)

    def _handle_upload(self, curr):
        try:
            content_type = self.headers.get('Content-Type')
            if not content_type or 'boundary=' not in content_type: return self.send_error(400)
            bnd = content_type.split('boundary=')[1].encode()
            rem = int(self.headers.get('Content-Length', 0))
            
            line = self.rfile.readline(); rem -= len(line)
            if bnd not in line: return self.send_error(400)

            while rem > 0:
                line = self.rfile.readline(); rem -= len(line)
                fn = re.findall(r'filename="(.*?)"', line.decode('utf-8', 'ignore'))
                if not fn:
                    if b'--' + bnd + b'--' in line: break
                    continue
                filename = fn[0]
                ...
                with open(os.path.join(curr, filename), 'wb') as f:
                    pline = self.rfile.readline(); rem -= len(pline)
                    while rem > 0:
                        line = self.rfile.readline(); rem -= len(line)
                        if bnd in line:
                            if pline.endswith(b'\r\n'): f.write(pline[:-2])
                            elif pline.endswith(b'\n'): f.write(pline[:-1])
                            else: f.write(pline)
                            break
                        f.write(pline); pline = line
                add_log(self.client_address[0], f"Uploaded: {filename}")
                if b'--' + bnd + b'--' in line: break
            self.send_response(200); self.end_headers()

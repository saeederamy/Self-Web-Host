...
def get_hub_size(path):
    ...
    now = time.time()
    # هر 15 ثانیه یک بار حجم رو آپدیت میکنه تا سایت کند نشه
...
<body>
    ...
    <div id="treeModal" class="modal">
        <div class="modal-content glass-box" style="flex-direction:column;padding:25px;max-width:500px;height:75%;">
            <h3 id="tree-title" style="margin:0 0 20px 0;color:var(--text-main)">Select Destination</h3>
    ...
    <div id="logModal" class="modal">
        <div class="modal-content glass-box" style="flex-direction:column;padding:25px;max-width:800px;height:85%;">
            ...
            <textarea readonly id="log-viewer" style="width:100%;height:100%;background:rgba(0,0,0,0.8);color:#10b981;border-radius:12px;padding:20px"></textarea>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content glass-box" style="flex-direction:column;padding:25px;max-width:800px;height:85%;">
            <h3 id="edit-name" style="color:var(--text-main)"></h3>
            <textarea id="edit-box" style="width:100%;height:100%;background:rgba(0,0,0,0.8);color:#f8fafc;padding:20px"></textarea>
            <div style="margin-top:20px;display:flex;gap:12px;justify-content:flex-end">
                <button class="btn" onclick="document.getElementById('editModal').style.display='none'">Cancel</button>
                <button class="btn action-orange" onclick="saveEdit()">💾 Save</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function askPath(a, t, e = {}) {
            ...
            fetch('/action', {
                ...
                if(l === "EXISTS") alert("⚠️ Path exists! Choose another.");
...
class FileHubHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _serve_ui(self, role, curr, req_dir):
        ...
        sa_btn = '<label style="color:var(--text-main);font-size:12px;background:var(--glass-bg);padding:6px;border-radius:8px;cursor:pointer;"><input type="checkbox" onchange="toggleAll(event)"> Select All</label>' if role == 'admin' else ''
        ...
        if role == 'admin':
            try:
                t, u, f = shutil.disk_usage(self.CONFIG['UPLOAD_DIR'])
                sz = get_hub_size(self.CONFIG['UPLOAD_DIR'])
                d_html = f'<div class="glass-box" style="display:flex;justify-content:space-around;padding:15px;margin-bottom:20px;text-align:center"><div>Space: <b>{format_size(t)}</b></div><div style="color:#10b981">Free: <b>{format_size(f)}</b></div><div style="color:var(--neon-orange)">Hub Size: <b>{format_size(sz)}</b></div></div>'
            except: 
                pass
